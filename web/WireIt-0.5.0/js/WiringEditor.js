...
WireIt.WiringEditor.prototype = {
	 ...
	 * @method updateModule
	 * Created by Diego for use on the Meican
	 */
	updateModule: function(id) {
			...
			this.adapter.updateWiring(id, this.tempSavedWiring, {
  ...
  * Created by Diego for use on the Meican
  */
 loadWorkflow: function(idWork) {
    this.adapter.listWirings(idWork,{
		success: function(result) {
			this.onLoadSuccess(result);
		},
		failure: function(errorStr) {
  ...
  * Edited by Diego for use on the Meican
  */
 onLoadSuccess: function(wiring) {
	this.pipe = wiring;
	this.loadPipe();
 },
 ...
 loadPipe: function() {	
	try {
		...
		if(this.pipe.name != null) {
    		try {
    			wiring = JSON.parse(this.pipe.working);
    		}
    		catch(ex) {
    			this.alert("Unable to eval working json for module "+name);
    			return null;
		...
		if(lang.isArray(wiring.modules)) {
			...
			for(i = 0 ; i < wiring.modules.length ; i++) {
				...
				if(this.modulesByName[m.name]) {
					...
					if(m.name == 'Hour'){
						var init = moment.utc({hour: m.value.post.init[0], minute: m.value.post.init[1]});
						init = init.clone().tz(timeZone);
						var finish = moment.utc({hour: m.value.post.finish[0], minute: m.value.post.finish[1]});
						finish = finish.clone().tz(timeZone);
						m.value.post.init[0] = init.get('hour').toString();
						m.value.post.init[1] = init.get('minute').toString();
						m.value.post.finish[0] = finish.get('hour').toString();
						m.value.post.finish[1] = finish.get('minute').toString();
						if(m.value.post.finish[0].length < 2) m.value.post.finish[0] = '0'.concat(m.value.post.finish[0]);
						if(m.value.post.finish[1].length < 2) m.value.post.finish[1] = '0'.concat(m.value.post.finish[1]);
						if(m.value.post.init[0].length < 2) m.value.post.init[0] = '0'.concat(m.value.post.init[0]);
						if(m.value.post.init[1].length < 2) m.value.post.init[1] = '0'.concat(m.value.post.init[1]);
					}	
