 ...
 */   
{
   ...
   onOkEditor: function() {
      ...
      if(typeof newValue == 'string') {
    	  var newText = this.editorField.getText();
    	  this.setValueAndText(newValue, newText);
      }
      else if(newValue['dom_operator'] != undefined){ //Na unidade dominio, para exibir corretamente.
    	  var newText = this.editorField.getText()
    	  this.setValueAndText(newValue, newText);
      }
      else{
   ...
   getText: function() {
	  var editorOpened = (this.editorContainer.style.display == '');
	  return editorOpened ? this.editorField.getText() : this.value;
   },
    ...
    */
   setValue: function(value, sendUpdatedEvt, abobora) {
	  this.value = value;
      ...
      //This part only is executed on load workflow, and it's responsible for show correctly the selected option.
      var text = value;
	  if(sendUpdatedEvt == false){
	      if(typeof this.editorField.getValue() == 'string') {
		  this.openEditor();
		  text = this.getText();
		  this.close();
		  inputEx.renderVisu(this.options.visu, text, this.formattedContainer);
	      }
	      else if(value['dom_operator'] != undefined){ //For Domain node shows correctly the selected option
	    	  this.openEditor();
			  text = this.getText();
			  this.close();
			  inputEx.renderVisu(this.options.visu, text, this.formattedContainer);
      ...
      inputEx.InPlaceEdit.superclass.setValue.call(this, text, sendUpdatedEvt);
   },
   
   setValueAndText: function(value, text, sendUpdatedEvt) {   
	      // Store the value
		  this.value = value;
...
inputEx.messages.emptyInPlaceEdit = tt("(click to edit)");
inputEx.messages.cancelEditor = tt("cancel");
